...
export class UserBusiness {
  ...
  public getUserProfile = async (token: any) => {
    const authenticator = new Authenticator();
  ...
  public createRecipe = async (input: any) => {
    const authenticator = new Authenticator();
    const payload = authenticator.getTokenPayload(input.token);
    console.log(payload);
    ...
    const userDatabase = new UserDatabase();
    const userDB = await userDatabase.createRecipe(input, id, payload.id);
